...
public final class TeamCommand {
    ...
    private static int disband(ServerCommandSource s) throws Exception { ServerPlayerEntity p = s.getPlayerOrThrow(); Team t = JustTeamsFabric.teams().getTeam(p.getUuid()); if (t == null || !t.isOwner(p.getUuid())) { s.sendError(Text.literal("You do not own a team.")); return 0; } for (TeamPlayer m : t.getMembers()) { TeamChatManager.disable(m.getPlayerUuid()); JustTeamsFabric.glow().stopGlowForPlayer(s.getServer(), m.getPlayerUuid()); } TeamEnderChestGui.closeAndRelease(s.getServer(), t); JustTeamsFabric.teams().unregister(t); JustTeamsFabric.storage().save(JustTeamsFabric.teams()); TeamNotificationManager.notifyDisband(s.getServer(), t, p.getUuid()); return 1; }
