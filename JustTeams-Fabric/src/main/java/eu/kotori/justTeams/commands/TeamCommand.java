...
public final class TeamCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("team")
                ...
                .then(CommandManager.literal("accept").then(CommandManager.argument("team", StringArgumentType.word())
                ...
                .then(CommandManager.literal("deny").then(CommandManager.argument("team", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_DENY,
                                () -> denyInvite(c.getSource(), StringArgumentType.getString(c, "team"))))))
                .then(CommandManager.literal("join").then(CommandManager.argument("team", StringArgumentType.word())
                        .executes(c -> run(c.getSource(), JustTeamsPermissions.COMMAND_JOIN,
                                () -> requestJoin(c.getSource(), StringArgumentType.getString(c, "team"))))))
                .then(CommandManager.literal("unjoin").then(CommandManager.argument("team", StringArgumentType.word())
    ...
    private static int invite(ServerCommandSource s,String playerName)throws Exception{ServerPlayerEntity p=s.getPlayerOrThrow();Team t=requireTeam(s,p);TeamPlayer member=t.getMember(p.getUuid());if(member==null||!member.canInvite()){s.sendError(Text.literal("You do not have permission to invite players."));return 0;}ServerPlayerEntity target=s.getServer().getPlayerManager().getPlayer(playerName);if(target==null){s.sendError(Text.literal("Player not found or is not online."));return 0;}if(target.getUuid().equals(p.getUuid())){s.sendError(Text.literal("You cannot invite yourself."));return 0;}if(JustTeamsFabric.teams().isInTeam(target.getUuid())){s.sendError(Text.literal("That player is already in a team."));return 0;}t.addInvite(target.getUuid());JustTeamsFabric.storage().save(JustTeamsFabric.teams());s.sendFeedback(()->prefix().append(Text.literal("You have invited ").setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false))).append(Text.literal(target.getName().getString()).setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false))).append(Text.literal(" to your team.").setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false))),false);target.sendMessage(prefix().append(Text.literal("You have been invited to join ").setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false))).append(Text.literal(t.getName()+". ").setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false))),false);return 1;}
    private static int acceptInvite(ServerCommandSource s,String teamName)throws Exception{ServerPlayerEntity p=s.getPlayerOrThrow();if(JustTeamsFabric.teams().isInTeam(p.getUuid())){s.sendError(prefix().append(Text.literal("You are already in a team. Leave your current one first.").setStyle(Style.EMPTY.withColor(Formatting.RED).withItalic(false))));return 0;}Team t=findTeam(teamName);if(t==null||!t.getInvites().contains(p.getUuid())){s.sendError(Text.literal("You do not have an invite from that team."));return 0;}t.removeInvite(p.getUuid());TeamPlayer m=new TeamPlayer(p.getUuid(),TeamRole.MEMBER,Instant.now(),false,false,false,false);m.setLastKnownName(p.getName().getString());JustTeamsFabric.teams().addMember(t,m);JustTeamsFabric.storage().save(JustTeamsFabric.teams());JustTeamsFabric.glow().refreshAll(s.getServer());p.sendMessage(prefix().append(Text.literal("You have joined the team "+t.getName()+".").setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false))),false);TeamNotificationManager.notifyJoinRequestAccepted(s.getServer(),t,p.getUuid());return 1;}
    private static int denyInvite(ServerCommandSource s,String teamName)throws Exception{ServerPlayerEntity p=s.getPlayerOrThrow();Team t=findTeam(teamName);if(t==null||!t.getInvites().contains(p.getUuid())){s.sendError(Text.literal("You do not have an invite from that team."));return 0;}t.removeInvite(p.getUuid());JustTeamsFabric.storage().save(JustTeamsFabric.teams());p.sendMessage(prefix().append(Text.literal("You have denied the invitation from "+t.getName()+".").setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false))),false);return 1;}
    private static int requestJoin(ServerCommandSource s,String teamName)throws Exception{ServerPlayerEntity p=s.getPlayerOrThrow();if(JustTeamsFabric.teams().isInTeam(p.getUuid())){s.sendError(prefix().append(Text.literal("You are already in a team. Leave your current one first.").setStyle(Style.EMPTY.withColor(Formatting.RED).withItalic(false))));return 0;}Team t=findTeam(teamName);if(t==null){s.sendError(Text.literal("A team with that name does not exist."));return 0;}t.addJoinRequest(p.getUuid());JustTeamsFabric.storage().save(JustTeamsFabric.teams());TeamNotificationManager.notifyJoinRequestSent(s.getServer(),p,t);TeamNotificationManager.notifyJoinRequest(s.getServer(),t,p.getUuid());return 1;}
    private static int cancelJoinRequest(ServerCommandSource s,String teamName)throws Exception{ServerPlayerEntity p=s.getPlayerOrThrow();Team t=findTeam(teamName);if(t==null){s.sendError(Text.literal("A team with that name does not exist."));return 0;}t.removeJoinRequest(p.getUuid());JustTeamsFabric.storage().save(JustTeamsFabric.teams());return 1;}
    ...
    private static Team findTeam(String name){return JustTeamsFabric.teams().getTeams().stream().filter(t->t.getName().equalsIgnoreCase(name)).findFirst().orElse(null);}
