...
public final class TeamCommandParityExtension {
    ...
    private static int executeInvite(ServerCommandSource source, String targetName) {
        try {
            ...
            Text received = prefix().append(Text.literal("You have been invited to join " + team.getName() + ". ").setStyle(white()))
    ...
    private static int executeLeave(ServerCommandSource source, String teamName) {
        try {
            ServerPlayerEntity player = source.getPlayerOrThrow(); requirePermission(player, JustTeamsPermissions.COMMAND_LEAVE);
            Team team = requireTeam(player);
            ...
            TeamPlayer member = team.getMember(player.getUuid()); if (member != null) { member.setAutoBankEnabled(false); member.setTeamChatEnabled(false); }
    ...
    private static int executeAccept(ServerCommandSource source, String teamName) {
        try {
            ServerPlayerEntity player = source.getPlayerOrThrow(); requirePermission(player, JustTeamsPermissions.COMMAND_ACCEPT);
            ...
            Team team = findTeam(teamName); if (team == null || !team.hasInvite(player.getUuid())) throw new IllegalStateException("You do not have a pending invite from this team.");
    ...
    private static int executeDeny(ServerCommandSource source, String teamName) {
        try {
            ServerPlayerEntity player = source.getPlayerOrThrow(); requirePermission(player, JustTeamsPermissions.COMMAND_DENY);
            Team team = findTeam(teamName); if (team == null || !team.hasInvite(player.getUuid())) throw new IllegalStateException("You do not have a pending invite from this team.");
    ...
    private static int executeJoin(ServerCommandSource source, String teamName) {
        try {
            ServerPlayerEntity player = source.getPlayerOrThrow(); requirePermission(player, JustTeamsPermissions.COMMAND_JOIN);
            if (JustTeamsFabric.teams().isInTeam(player.getUuid())) throw new IllegalStateException("You are already in a team. Leave your current one first.");
            Team team = findTeam(teamName); if (team == null) throw new IllegalStateException("A team with that name does not exist.");
            team.addJoinRequest(player.getUuid()); JustTeamsFabric.storage().save(JustTeamsFabric.teams());
    ...
    private static int executeUnjoin(ServerCommandSource source, String teamName) {
        try {
            ServerPlayerEntity player = source.getPlayerOrThrow(); requirePermission(player, JustTeamsPermissions.COMMAND_UNJOIN);
            Team team = findTeam(teamName); if (team == null) throw new IllegalStateException("A team with that name does not exist.");
    ...
    private static int executePvp(ServerCommandSource source) {
        try {
            ServerPlayerEntity player = source.getPlayerOrThrow(); requirePermission(player, JustTeamsPermissions.COMMAND_PVP); Team team = requireTeam(player);
    ...
    private static int executeChat(ServerCommandSource source) {
        try {
            ServerPlayerEntity player = source.getPlayerOrThrow(); requirePermission(player, JustTeamsPermissions.COMMAND_CHAT); Team team = requireTeam(player); TeamPlayer member = team.getMember(player.getUuid());
    ...
    private static int failure(ServerCommandSource source, Exception exception) { source.sendMessage(prefix().append(Text.literal(exception.getMessage() == null ? "Command failed." : exception.getMessage()).setStyle(Style.EMPTY.withColor(Formatting.RED).withItalic(false))), false); return 0; }
