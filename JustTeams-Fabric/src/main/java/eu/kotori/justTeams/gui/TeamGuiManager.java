...
public final class TeamGuiManager {
    ...
    public static void openMain(PlayerEntity player){
        ...
        player.openHandledScreen(new net.minecraft.screen.SimpleNamedScreenHandlerFactory((syncId,inventory,ignored)->new TeamMenuHandler(syncId,inventory,player.getUuid(),team,TeamGuiManager::handleMainClick),Text.literal("ᴛᴇᴀᴍ - "+team.getMembers().size()).setStyle(Style.EMPTY.withItalic(false))));
    ...
    public static void openPersistentView(ServerPlayerEntity player,TeamInPlaceGui.View view){Team team=JustTeamsFabric.teams().getTeam(player.getUuid());if(team==null){TeamPersistentNoTeamGui.openMain(player);return;}TeamMenuHandler menu;if(player.currentScreenHandler instanceof TeamMenuHandler existing&&existing.getTeam().getId()==team.getId()&&team.isMember(player.getUuid()))menu=existing;else{openMain(player);if(!(player.currentScreenHandler instanceof TeamMenuHandler opened))return;menu=opened;}TeamPersistentWarpManagementGui.close(menu);TeamPersistentBlacklistGui.close(menu);TeamPersistentLeaderboardGui.close(menu);TeamBankLogsGui.close(menu);switch(view){case MAIN->TeamInPlaceGui.returnToMain(menu);case JOIN_REQUESTS->TeamInPlaceGui.enterJoinRequests(menu,player,team);case WARPS->TeamInPlaceGui.enterWarps(menu,player,team);case SETTINGS->TeamInPlaceGui.enterSettings(menu,player,team);}}
    public static void openPersistentLeaderboard(ServerPlayerEntity player,TeamPersistentLeaderboardGui.View view,TeamPersistentLeaderboardGui.TeamPersistentLeaderboardGuiType type){Team team=JustTeamsFabric.teams().getTeam(player.getUuid());if(team==null){if(view==TeamPersistentLeaderboardGui.View.CATEGORIES)TeamPersistentNoTeamGui.openLeaderboardCategories(player);else TeamPersistentNoTeamGui.openLeaderboard(player,type);return;}TeamMenuHandler menu;if(player.currentScreenHandler instanceof TeamMenuHandler existing&&existing.getTeam().getId()==team.getId()&&team.isMember(player.getUuid()))menu=existing;else{openMain(player);if(!(player.currentScreenHandler instanceof TeamMenuHandler opened))return;menu=opened;}TeamPersistentWarpManagementGui.close(menu);TeamPersistentBlacklistGui.close(menu);TeamBankLogsGui.close(menu);if(view==TeamPersistentLeaderboardGui.View.CATEGORIES)TeamPersistentLeaderboardGui.openCategories(menu);else TeamPersistentLeaderboardGui.openLeaderboard(menu,type);}
    ...
    private static void handleMainClick(PlayerEntity player,int slot,int button,SlotActionType actionType,Team team,TeamMenuHandler menu){
        ...
        if(TeamDisbandConfirmationGui.isOpen(menu))return;
