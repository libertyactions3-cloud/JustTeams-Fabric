...
public final class TeamGuiManager {
    ...
    public static void openMain(PlayerEntity player) {
        ...
        if (team == null) { if (player instanceof ServerPlayerEntity serverPlayer) TeamPersistentNoTeamGui.openMain(serverPlayer); return; }
        if (player.currentScreenHandler instanceof TeamMenuHandler menu && menu.getTeam().getId() == team.getId() && team.isMember(player.getUuid())) {
            TeamPersistentWarpManagementGui.close(menu); TeamPersistentBlacklistGui.close(menu); TeamPersistentLeaderboardGui.close(menu); TeamInPlaceMemberGui.back(menu); TeamBankLogsGui.close(menu); TeamInPlaceGui.returnToMain(menu); TeamInPlaceGui.refreshMainMembers(menu, player, team); return;
        }
        player.openHandledScreen(new net.minecraft.screen.SimpleNamedScreenHandlerFactory(
                ...
                Text.literal("ᴛᴇᴀᴍ - " + team.getMembers().size()).setStyle(Style.EMPTY.withItalic(false))));
    ...
    public static void openPersistentView(ServerPlayerEntity player, TeamInPlaceGui.View view) {
        Team team = JustTeamsFabric.teams().getTeam(player.getUuid()); if (team == null) { TeamPersistentNoTeamGui.openMain(player); return; }
        ...
        switch (view) { case MAIN -> TeamInPlaceGui.returnToMain(menu); case JOIN_REQUESTS -> TeamInPlaceGui.enterJoinRequests(menu, player, team); case WARPS -> TeamInPlaceGui.enterWarps(menu, player, team); case SETTINGS -> TeamInPlaceGui.enterSettings(menu, player, team); }
    }

    public static void openPersistentLeaderboard(ServerPlayerEntity player, TeamPersistentLeaderboardGui.View view, TeamPersistentLeaderboardGui.TeamPersistentLeaderboardGuiType type) {
        Team team = JustTeamsFabric.teams().getTeam(player.getUuid()); if (team == null) { if (view == TeamPersistentLeaderboardGui.View.CATEGORIES) TeamPersistentNoTeamGui.openLeaderboardCategories(player); else TeamPersistentNoTeamGui.openLeaderboard(player, type); return; }
        TeamMenuHandler menu;
        if (player.currentScreenHandler instanceof TeamMenuHandler existing && existing.getTeam().getId() == team.getId() && team.isMember(player.getUuid())) menu = existing;
        else { openMain(player); if (!(player.currentScreenHandler instanceof TeamMenuHandler opened)) return; menu = opened; }
        TeamPersistentWarpManagementGui.close(menu); TeamPersistentBlacklistGui.close(menu); TeamBankLogsGui.close(menu); if (view == TeamPersistentLeaderboardGui.View.CATEGORIES) TeamPersistentLeaderboardGui.openCategories(menu); else TeamPersistentLeaderboardGui.openLeaderboard(menu, type);
    }
    ...
    private static void handleMainClick(PlayerEntity player, int slot, int button, SlotActionType actionType, Team team, TeamMenuHandler menu) {
        ...
        if (memberIndex >= 0) {
            List<TeamPlayer> displayedMembers = orderedMembers(player, team); if (memberIndex >= displayedMembers.size()) return;
            TeamPlayer target = displayedMembers.get(memberIndex); if (target.getPlayerUuid().equals(player.getUuid())) return;
            TeamPlayer viewerMember = team.getMember(player.getUuid()); if (viewerMember == null || viewerMember.getRank().ordinal() < TeamRank.UNDEROFFICER.ordinal()) return;
            if (target.getRank().ordinal() >= viewerMember.getRank().ordinal()) return;
            TeamInPlaceMemberGui.enter(menu, player, team, target, slot); return;
        }

        switch (slot) {
            case 45 -> togglePvp(player, team, menu);
            case 53 -> { if (player instanceof ServerPlayerEntity sp && team.isOwner(player.getUuid())) TeamDisbandConfirmationGui.openFirst(menu, sp, team); else leaveTeam(player, team); }
    ...
    private static List<TeamPlayer> orderedMembers(PlayerEntity viewer, Team team) {
        List<TeamPlayer> members = new ArrayList<>(team.getMembers()); var server = viewer instanceof ServerPlayerEntity sp ? sp.getEntityWorld().getServer() : null;
        Comparator<TeamPlayer> comparator = switch (team.getCurrentSortType()) {
            case ONLINE_STATUS -> Comparator.comparing((TeamPlayer m) -> server != null && server.getPlayerManager().getPlayer(m.getPlayerUuid()) != null).reversed().thenComparing(m -> PlayerNameResolver.resolve(server, m.getPlayerUuid()), String.CASE_INSENSITIVE_ORDER);
            case RANK -> Comparator.comparingInt((TeamPlayer m) -> m.getRank().ordinal()).reversed().thenComparing(m -> PlayerNameResolver.resolve(server, m.getPlayerUuid()), String.CASE_INSENSITIVE_ORDER);
    ...
    private static void useHome(PlayerEntity player, Team team) {
        if (!(player instanceof ServerPlayerEntity serverPlayer)) return; TeamPlayer member = team.getMember(player.getUuid());
        ...
        if (team.getHome() == null) { Text message = Text.literal("[ᴛᴇᴀᴍꜱ] ").setStyle(Style.EMPTY.withColor(0x4C9DDE).withItalic(false)).append(Text.literal("Your team does not have a home set. An Owner or Co-Owner can set one with /team sethome.").setStyle(Style.EMPTY.withColor(Formatting.RED).withItalic(false))); player.sendMessage(message, false); return; }
        JustTeamsFabric.teleports().requestHome(serverPlayer, team.getHome());
    }
    private static void togglePvp(PlayerEntity player, Team team, TeamMenuHandler menu) { if (!team.isOwner(player.getUuid())) { player.sendMessage(Text.literal("Only the team owner can change PvP."), true); return; } team.setPvpEnabled(!team.isPvpEnabled()); save(); TeamInPlaceGui.updateMainPvpItem(menu, team); player.sendMessage(Text.literal(team.isPvpEnabled() ? "[ᴛᴇᴀᴍꜱ] Team PvP has been enabled." : "[ᴛᴇᴀᴍꜱ] Team PvP has been disabled.").setStyle(Style.EMPTY.withColor(team.isPvpEnabled() ? Formatting.GREEN : Formatting.RED).withItalic(false)), false); }
    private static void leaveTeam(PlayerEntity player, Team team) { String name = player.getName().getString(); TeamChatManager.disable(player.getUuid()); if (player instanceof ServerPlayerEntity sp) { var server = sp.getEntityWorld().getServer(); TeamEnderChestGui.closeViewer(server, team, player.getUuid()); JustTeamsFabric.glow().stopGlowForPlayer(server, player.getUuid()); for (TeamPlayer member : team.getMembers()) { ServerPlayerEntity online = server.getPlayerManager().getPlayer(member.getPlayerUuid()); if (online != null) online.sendMessage(Text.literal("[ᴛᴇᴀᴍꜱ] " + name + "'s has left the team.").setStyle(Style.EMPTY.withColor(Formatting.WHITE).withItalic(false)), false); } } JustTeamsFabric.teams().removeMember(team, player.getUuid()); save(); close(player); }
    public static void performDisband(ServerPlayerEntity player, Team team) { if (player == null || team == null || !team.isOwner(player.getUuid())) return; var server = player.getEntityWorld().getServer(); TeamEnderChestGui.closeAndRelease(server, team); for (TeamPlayer member : team.getMembers()) { TeamChatManager.disable(member.getPlayerUuid()); JustTeamsFabric.glow().stopGlowForPlayer(server, member.getPlayerUuid()); } JustTeamsFabric.teams().unregister(team); save(); player.closeHandledScreen(); player.sendMessage(Text.literal("[ᴛᴇᴀᴍꜱ] You have successfully disbanded your team.").setStyle(Style.EMPTY.withColor(Formatting.GREEN).withItalic(false)), false); }
