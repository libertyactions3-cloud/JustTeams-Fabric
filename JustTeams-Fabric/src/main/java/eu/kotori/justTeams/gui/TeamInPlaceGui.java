...
public final class TeamInPlaceGui {
    ...
    private enum View { MAIN, JOIN_REQUESTS, WARPS }
    ...
    public static View view(TeamMenuHandler menu) {
        return VIEWS.getOrDefault(menu, View.MAIN);
    }

    public static void enterJoinRequests(TeamMenuHandler menu, PlayerEntity player, Team team) {
        snapshotMain(menu);
        VIEWS.put(menu, View.JOIN_REQUESTS);
        Inventory inventory = menu.getMenuInventory();
        clearForSubmenu(inventory);
        ...
        for (int i = 0; i < JOIN_REQUEST_SLOTS.length && i < requests.size(); i++) {
            inventory.setStack(JOIN_REQUEST_SLOTS[i], requestItem(player, requests.get(i)));
        }
    ...
    public static void enterWarps(TeamMenuHandler menu, PlayerEntity player, Team team) {
        ...
        for (int i = 0; i < WARP_SLOTS.length && i < warps.size(); i++) {
            inventory.setStack(WARP_SLOTS[i], warpItem(warps.get(i)));
        }
    ...
    public static void returnToMain(TeamMenuHandler menu) {
        ...
        for (int slot = 0; slot < snapshot.length; slot++) {
            inventory.setStack(slot, snapshot[slot].copy());
        }
    ...
    public static boolean handleWarpClick(TeamMenuHandler menu, ServerPlayerEntity player, Team team, int slot, int button) {
        int index = indexOf(WARP_SLOTS, slot);
        if (index < 0 || index >= team.getWarps().size()) return false;
        TeamWarp warp = team.getWarps().get(index);
        if (button == 1 && team.hasElevatedPermissions(player.getUuid())) {
            // Warp editing still uses the existing management/input flow; the parent menu itself remains in place.
            TeamWarpManagementGui.open(player, team, warp);
            return true;
        }
        if (!warp.isEnabled()) {
            player.sendMessage(Text.literal("That warp is disabled."), true);
            return true;
        }
    ...
    private static void notifyPlayer(PlayerEntity viewer, UUID targetUuid, String message) {
        if (viewer instanceof ServerPlayerEntity serverPlayer) {
            ServerPlayerEntity target = serverPlayer.getEntityWorld().getServer().getPlayerManager().getPlayer(targetUuid);
    ...
    private static ItemStack requestItem(PlayerEntity viewer, UUID uuid) {
        ...
        if (viewer instanceof ServerPlayerEntity serverPlayer) {
            ...
            if (target != null) { online = true; name = target.getName().getString(); }
        }
        MutableText title = Text.empty();
        title.append(Text.literal("● ").setStyle(Style.EMPTY.withColor(online ? Formatting.GREEN : Formatting.RED).withItalic(false)));
    ...
    private static ItemStack warpItem(TeamWarp warp) {
        ...
        int separator = warp.getWorld().lastIndexOf(':');
        String serverName = separator >= 0 && separator + 1 < warp.getWorld().length()
                ? warp.getWorld().substring(separator + 1) : warp.getWorld();
        stack.set(DataComponentTypes.LORE, new LoreComponent(List.of(
                composeLine("Server: ", serverName, Formatting.GRAY, Formatting.WHITE),
                plainLine("", Formatting.GRAY),
                passwordProtected ? plainLine("Password Protected", Formatting.RED) : plainLine("Public", Formatting.GREEN)
        )));
        return stack;
    }
    ...
    private static MutableText sortLine(String name, boolean selected) {
        return Text.literal("▪ " + name).setStyle(Style.EMPTY.withColor(selected ? Formatting.GREEN : Formatting.GRAY).withItalic(false));
    }
    ...
    private static int indexOf(int[] values, int slot) {
        for (int i = 0; i < values.length; i++) if (values[i] == slot) return i;
        return -1;
    }

    private static void save() {
        try { JustTeamsFabric.storage().save(JustTeamsFabric.teams()); }
        catch (IOException e) { JustTeamsFabric.LOGGER.error("Failed to save in-place team GUI change", e); }
    }
}
