...
public final class TeamInPlaceGui {
    ...
    public static void returnToMain(TeamMenuHandler menu) {
        ItemStack[] snapshot = MAIN_SNAPSHOTS.get(menu); if (snapshot == null) return;
        Inventory inventory = menu.getMenuInventory(); for (int slot = 0; slot < snapshot.length; slot++) inventory.setStack(slot, snapshot[slot].copy());
        VIEWS.put(menu, View.MAIN); menu.sendContentUpdates();
    }
    ...
    public static boolean handleJoinRequestClick(TeamMenuHandler menu, PlayerEntity player, Team team, int slot, int button) {
        ...
        if (button == 0) { team.removeJoinRequest(uuid); if (!JustTeamsFabric.teams().isInTeam(uuid)) {
                JustTeamsFabric.teams().addMember(team, new TeamPlayer(uuid, eu.kotori.justTeams.team.TeamRole.MEMBER, java.time.Instant.now(), false, false, false, true));
    ...
    public static boolean handleWarpClick(TeamMenuHandler menu, ServerPlayerEntity player, Team team, int slot, int button) {
        ...
        if (!warp.getPassword().isEmpty()) {
            TeamStringInputGui.open(player, "Warp Password", "Enter password", value -> {
                if (!warp.getPassword().equals(value)) { player.sendMessage(Text.literal("Incorrect warp password."), true); enterWarps(menu, player, team); return; }
                requestWarp(player, warp);
            }, () -> enterWarps(menu, player, team)); return true;
        }
    ...
    public static boolean handleSettingsClick(TeamMenuHandler menu, ServerPlayerEntity player, Team team, int slot) {
        ...
        switch (slot) {
            case 11 -> ChatInputManager.begin(player, "Enter the new team tag (1-4 characters, or type cancel):", input -> {
                try { JustTeamsFabric.teams().setTag(player.getUuid(), input); save(); player.sendMessage(Text.literal("Team tag updated."), false); }
                catch (IllegalArgumentException | IllegalStateException exception) { player.sendMessage(Text.literal(exception.getMessage()), false); }
                enterSettings(menu, player, team);
            });
            case 13 -> ChatInputManager.begin(player, "Enter the new team description (1-256 characters, or type cancel):", input -> {
                try { JustTeamsFabric.teams().setDescription(player.getUuid(), input); save(); player.sendMessage(Text.literal("Team description updated."), false); }
                catch (IllegalArgumentException | IllegalStateException exception) { player.sendMessage(Text.literal(exception.getMessage()), false); }
                enterSettings(menu, player, team);
            });
            case 15 -> { try { boolean enabled = JustTeamsFabric.teams().togglePublic(player.getUuid()); save(); player.sendMessage(Text.literal("Team is now " + (enabled ? "public" : "private") + "."), false); enterSettings(menu, player, team); }
                catch (IllegalStateException exception) { player.sendMessage(Text.literal(exception.getMessage()), false); } }
            default -> { }
        }
        return true;
    }

    private static void requestWarp(ServerPlayerEntity player, TeamWarp warp) {
        JustTeamsFabric.teleports().requestWarp(player, new TeamLocation(warp.getWorld(), warp.getX(), warp.getY(), warp.getZ(), warp.getYaw(), warp.getPitch()), warp.getCost());
    }
    private static void notifyPlayer(PlayerEntity viewer, UUID targetUuid, String message) {
        if (viewer instanceof ServerPlayerEntity serverPlayer) { ServerPlayerEntity target = serverPlayer.getEntityWorld().getServer().getPlayerManager().getPlayer(targetUuid); if (target != null) target.sendMessage(Text.literal(message), false); }
    }
    private static void snapshotMain(TeamMenuHandler menu) {
        if (MAIN_SNAPSHOTS.containsKey(menu)) return; ItemStack[] snapshot = new ItemStack[54]; for (int slot = 0; slot < snapshot.length; slot++) snapshot[slot] = menu.getMenuInventory().getStack(slot).copy(); MAIN_SNAPSHOTS.put(menu, snapshot);
    }
    private static void clearForSubmenu(Inventory inventory) {
        ItemStack filler = namedPlain(Items.GRAY_STAINED_GLASS_PANE, " "); for (int slot = 0; slot < 54; slot++) inventory.setStack(slot, ItemStack.EMPTY); for (int slot = 0; slot < 9; slot++) inventory.setStack(slot, filler.copy()); for (int slot = 45; slot < 54; slot++) inventory.setStack(slot, filler.copy());
    }
    private static ItemStack requestItem(PlayerEntity viewer, UUID uuid) {
        ItemStack head = new ItemStack(Items.PLAYER_HEAD); head.set(DataComponentTypes.PROFILE, ProfileComponent.ofDynamic(uuid)); boolean online = false; String name = uuid.toString().substring(0, 8);
        if (viewer instanceof ServerPlayerEntity serverPlayer) { ServerPlayerEntity target = serverPlayer.getEntityWorld().getServer().getPlayerManager().getPlayer(uuid); if (target != null) { online = true; name = target.getName().getString(); } }
        MutableText title = Text.empty(); title.append(Text.literal("● ").setStyle(Style.EMPTY.withColor(online ? Formatting.GREEN : Formatting.RED).withItalic(false))); title.append(gradientText(name, true)); head.set(DataComponentTypes.CUSTOM_NAME, title);
        head.set(DataComponentTypes.LORE, new LoreComponent(List.of(plainLine("This player wants to join your team.", Formatting.GRAY), plainLine("", Formatting.GRAY), plainLine("Left-Click to Accept", Formatting.GREEN), plainLine("Right-Click to Deny", Formatting.RED)))); return head;
    }
    private static ItemStack warpItem(TeamWarp warp) {
        boolean passwordProtected = !warp.getPassword().isEmpty(); ItemStack stack = new ItemStack(passwordProtected ? Items.IRON_BLOCK : Items.GOLD_BLOCK); stack.set(DataComponentTypes.CUSTOM_NAME, gradientText(warp.getName(), true));
        int separator = warp.getWorld().lastIndexOf(':'); String serverName = separator >= 0 && separator + 1 < warp.getWorld().length() ? warp.getWorld().substring(separator + 1) : warp.getWorld();
        stack.set(DataComponentTypes.LORE, new LoreComponent(List.of(composeLine("Server: ", serverName, Formatting.GRAY, Formatting.WHITE), plainLine("", Formatting.GRAY), passwordProtected ? plainLine("Password Protected", Formatting.RED) : plainLine("Public", Formatting.GREEN)))); return stack;
    }
    ...
    private static MutableText gradientText(String value, boolean bold) {
        MutableText result = Text.empty(); if (value.isEmpty()) return result; int length = Math.max(1, value.codePointCount(0, value.length()) - 1); int index = 0;
        for (int offset = 0; offset < value.length();) { int codePoint = value.codePointAt(offset); double t = (double) index / length; int sr = (PRIMARY_START >> 16) & 0xFF, sg = (PRIMARY_START >> 8) & 0xFF, sb = PRIMARY_START & 0xFF; int er = (PRIMARY_END >> 16) & 0xFF, eg = (PRIMARY_END >> 8) & 0xFF, eb = PRIMARY_END & 0xFF; int r = (int) Math.round(sr + (er - sr) * t), g = (int) Math.round(sg + (eg - sg) * t), b = (int) Math.round(sb + (eb - sb) * t); int rgb = (r << 16) | (g << 8) | b; result.append(Text.literal(new String(Character.toChars(codePoint))).setStyle(Style.EMPTY.withColor(rgb).withBold(bold).withItalic(false))); offset += Character.charCount(codePoint); index++; }
        return result;
    }
