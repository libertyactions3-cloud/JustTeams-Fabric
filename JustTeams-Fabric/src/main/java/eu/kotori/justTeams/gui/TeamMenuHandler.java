...
public final class TeamMenuHandler extends ScreenHandler {
    ...
    private void populate(PlayerEntity viewer) {
        ...
        boolean bankPermission = viewer instanceof ServerPlayerEntity serverPlayer && JustTeamsFabric.permissions().has(serverPlayer, JustTeamsPermissions.COMMAND_BANK);
        ...
        if (elevated) {
            ...
            menuInventory.setStack(8, itemWithLore(namedColored(Items.LANTERN, "ᴊᴏɪɴ ʀᴇǫᴜᴇsᴛs", Formatting.RED, true), List.of(plainLine("Only the owner or co-owners can access this.", Formatting.RED))));
        }
        ...
        if (!bankEnabled || !bankPermission) {
            ...
            bank.set(DataComponentTypes.LORE, new LoreComponent(List.of(composeLine("Balance: ", String.format("%.2f", team.getBalance()), Formatting.GRAY, Formatting.WHITE), plainLine("", Formatting.GRAY), plainLine("Click to manage the bank.", Formatting.YELLOW))));
        }
        ...
        boolean enderPermission = viewerMember != null && (viewerMember.canUseEnderChest() || (viewer instanceof ServerPlayerEntity serverPlayer && JustTeamsFabric.permissions().has(serverPlayer, "justteams.bypass.enderchest.use")));
        ...
        if (!enderEnabled || !enderPermission) {
            ...
            enderChest.set(DataComponentTypes.LORE, new LoreComponent(List.of(plainLine("A shared inventory for your team.", Formatting.GRAY), plainLine("", Formatting.GRAY), plainLine("Click to open the ender chest.", Formatting.YELLOW))));
        }
        ...
        sort.set(DataComponentTypes.LORE, new LoreComponent(List.of(plainLine("Click to change the sorting.", Formatting.GRAY), plainLine("", Formatting.GRAY), sortLine("Join Date", currentSort == TeamSortType.JOIN_DATE), sortLine("Alphabetical", currentSort == TeamSortType.ALPHABETICAL), sortLine("Online Status", currentSort == TeamSortType.ONLINE_STATUS))));
        menuInventory.setStack(49, sort);

        ItemStack settings;
        if (elevated) {
            settings = namedGradient(Items.COMPARATOR, "ᴛᴇᴀᴍ sᴇᴛᴛɪɴɢs");
            settings.set(DataComponentTypes.LORE, new LoreComponent(List.of(plainLine("Click to manage team settings.", Formatting.YELLOW))));
        } else {
            settings = namedColored(Items.COMPARATOR, "ᴛᴇᴀᴍ sᴇᴛᴛɪɴɢs", Formatting.RED, true);
            settings.set(DataComponentTypes.LORE, new LoreComponent(List.of(plainLine("Only the owner or co-owners can access this.", Formatting.RED))));
        }
        ...
        ItemStack leaveOrDisband = namedColored(owner ? Items.TNT : Items.DARK_OAK_DOOR, owner ? "ᴅɪsʙᴀɴᴅ ᴛᴇᴀᴍ" : "ʟᴇᴀᴠᴇ ᴛᴇᴀᴍ", Formatting.RED, true);
    ...
    private List<TeamPlayer> orderedMembers() {
        ...
        remaining.sort(Comparator.comparing(TeamPlayer::getJoinDate, Comparator.nullsLast(Comparator.naturalOrder())));
    ...
    private ItemStack createMemberHead(PlayerEntity viewer, TeamPlayer member) {
        ...
        String roleIcon = switch (member.getRole()) { case OWNER -> "★ "; case CO_OWNER -> "◆ "; case MEMBER -> "● "; };
        name.append(Text.literal(roleIcon).setStyle(Style.EMPTY.withColor(roleColor(member.getRole())).withItalic(false)));
        if (isOnline) name.append(gradientText(playerName, false)); else name.append(Text.literal(playerName).setStyle(Style.EMPTY.withColor(0x808080).withItalic(false)));
        head.set(DataComponentTypes.CUSTOM_NAME, name);
        String roleName = switch (member.getRole()) { case OWNER -> "Owner"; case CO_OWNER -> "Co-Owner"; case MEMBER -> "Member"; };
        ...
        head.set(DataComponentTypes.LORE, new LoreComponent(List.of(composeLine("Role: ", roleName, Formatting.GRAY, Formatting.WHITE), composeLine("Joined: ", joined, Formatting.GRAY, Formatting.WHITE), composeLine("Server: ", serverName, Formatting.GRAY, Formatting.WHITE))));
        if (member.getRole() == TeamRole.OWNER) head.set(DataComponentTypes.ENCHANTMENT_GLINT_OVERRIDE, true);
        return head;
    }

    private static int roleColor(TeamRole role) { return switch (role) { case OWNER -> 0xFFD700; case CO_OWNER -> 0xFF6B6B; case MEMBER -> 0xFFFFFF; }; }
