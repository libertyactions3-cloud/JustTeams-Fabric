...
public final class TeamMenuHandler extends ScreenHandler {
    ...
    private void populate(PlayerEntity viewer) {
        ...
        menuInventory.setStack(8,itemWithLore(elevated?namedGradient(Items.SOUL_LANTERN,"ᴊᴏɪɴ ʀᴇǫᴜᴇs"):namedColored(Items.LANTERN,"ᴊᴏɪɴ ʀᴇǫᴜᴇs",Formatting.RED,true),List.of(plainLine(elevated?"View pending requests to join your team.":"Only the owner or co-owners can access this.",elevated?Formatting.GRAY:Formatting.RED))));
        ItemStack bank=namedGradient(Items.SUNFLOWER,"ᴛᴇᴀᴍ ʙᴀɴᴋ"); bank.set(DataComponentTypes.LORE,new LoreComponent(List.of(plainLine(team.getBank().getTotalEmeraldValue()+" total emeralds",Formatting.GREEN),plainLine("",Formatting.GRAY),plainLine("Click to manage the bank.",Formatting.YELLOW)))); menuInventory.setStack(50,bank);
    ...
    private List<TeamPlayer> orderedMembers(PlayerEntity viewer){List<TeamPlayer> members=new ArrayList<>(team.getMembers());MinecraftServer server=viewer instanceof ServerPlayerEntity sp?sp.getEntityWorld().getServer():null;Comparator<TeamPlayer> c=switch(team.getCurrentSortType()){case ONLINE_STATUS->Comparator.comparing((TeamPlayer m)->server!=null&&server.getPlayerManager().getPlayer(m.getPlayerUuid())!=null).reversed().thenComparing(m->PlayerNameResolver.resolve(server,m.getPlayerUuid()),String.CASE_INSENSITIVE_ORDER);case RANK->Comparator.comparingInt((TeamPlayer m)->m.getRank().ordinal()).reversed().thenComparing(m->PlayerNameResolver.resolve(server,m.getPlayerUuid()),String.CASE_INSENSITIVE_ORDER);case ALPHABETICAL->Comparator.comparing(m->PlayerNameResolver.resolve(server,m.getPlayerUuid()),String.CASE_INSENSITIVE_ORDER);case JOIN_DATE->Comparator.comparing(TeamPlayer::getJoinDate,Comparator.nullsLast(Comparator.naturalOrder()));};members.sort(c);return members;}
    private ItemStack createMemberHead(PlayerEntity viewer,TeamPlayer member){ItemStack head=new ItemStack(Items.PLAYER_HEAD);head.set(DataComponentTypes.PROFILE,ProfileComponent.ofDynamic(member.getPlayerUuid()));MinecraftServer server=viewer instanceof ServerPlayerEntity sp?sp.getEntityWorld().getServer():null;boolean online=server!=null&&server.getPlayerManager().getPlayer(member.getPlayerUuid())!=null;String name=PlayerNameResolver.resolve(server,member.getPlayerUuid());String symbol=switch(member.getRank()){case INITIATE->"+";case MEMBER->"›";case ASSOCIATE->"»";case UNDEROFFICER->"*";case OFFICER->"⁑";case CO_LEADER->"⁂";case LEADER->"★";};MutableText title=Text.literal("●").setStyle(Style.EMPTY.withColor(online?Formatting.GREEN:Formatting.RED).withItalic(false)).append(Text.literal("   ").setStyle(Style.EMPTY.withItalic(false))).append(Text.literal(symbol).setStyle(Style.EMPTY.withColor(Formatting.WHITE).withBold(false).withItalic(false))).append(Text.literal(" ").setStyle(Style.EMPTY.withItalic(false))).append(gradientText(name,false));head.set(DataComponentTypes.CUSTOM_NAME,title);String joined=member.getJoinDate()==null?"Unknown":DateTimeFormatter.ofPattern("dd MMM yyyy").withZone(ZoneOffset.UTC).format(member.getJoinDate());head.set(DataComponentTypes.LORE,new LoreComponent(List.of(composeLine("Rank: ",member.getRank().getDisplayName(),Formatting.GRAY,Formatting.WHITE),composeLine("Joined: ",joined,Formatting.GRAY,Formatting.WHITE))));return head;}
