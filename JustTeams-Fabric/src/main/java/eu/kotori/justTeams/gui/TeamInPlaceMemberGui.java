...
public final class TeamInPlaceMemberGui {
    ...
    public static boolean handle(TeamMenuHandler menu, PlayerEntity player, Team team, int slot) {
        ...
        switch (slot) {
            ...
            case 40 -> target.setCanUseAutoBank(!target.canUseAutoBank());
    ...
    private static void renderEditor(TeamMenuHandler menu, PlayerEntity viewer, Team team, TeamPlayer target) {
        ...
        inventory.setStack(13, actionItem(Items.GOLDEN_HELMET, "Rank: " + target.getRank().getDisplayName(), List.of(
                plainLine("Current rank: " + target.getRank().getDisplayName(), Formatting.GRAY),
                plainLine("", Formatting.GRAY),
                plainLine("Promote and demote with the dyes below.", Formatting.YELLOW))));

        inventory.setStack(19, target.getRank() == TeamRank.CO_LEADER || target.getRank() == TeamRank.LEADER
                ? lockedItem(Items.GRAY_DYE, "PROMOTE", "This rank cannot be promoted further.")
                : actionItem(Items.LIME_DYE, "PROMOTE TO " + target.getRank().promote().getDisplayName().toUpperCase(), List.of(
        ...
        inventory.setStack(20, target.getRank() == TeamRank.LEADER || target.getRank() == TeamRank.INITIATE
                ? lockedItem(Items.RED_DYE, "DEMOTE", "This rank cannot be demoted further.")
                : actionItem(Items.RED_DYE, "DEMOTE TO " + target.getRank().demote().getDisplayName().toUpperCase(), List.of(
        ...
        inventory.setStack(37, permissionItem(Items.GOLD_INGOT, "ʙᴀɴᴋ ᴡɪᴛʜᴅʀᴀᴡ", "Allow this member to withdraw from the team bank.", target.canWithdraw()));
        ...
        inventory.setStack(40, permissionItem(Items.EMERALD_BLOCK, "ᴛᴇᴀᴍ ᴀᴜᴛᴏʙᴀɴᴋ", "Allow currency costs to use the team bank automatically.", target.canUseAutoBank()));
    ...
    private static void renderTransferConfirmation(TeamMenuHandler menu, PlayerEntity viewer, Team team, TeamPlayer target) {
        ...
        inventory.setStack(4, actionItem(Items.BEACON, "TRANSFER OWNERSHIP", List.of(plainLine("Transfer ownership to " + resolveName(viewer, target) + "?", Formatting.WHITE), plainLine("This cannot be undone without another transfer.", Formatting.RED))));
        inventory.setStack(21, actionItem(Items.GREEN_WOOL, "CONFIRM", List.of(plainLine("Transfer ownership.", Formatting.YELLOW))));
        inventory.setStack(23, actionItem(Items.RED_WOOL, "CANCEL", List.of(plainLine("Return to member management.", Formatting.YELLOW))));
    ...
    private static void setOwnerPermissions(TeamPlayer member) {
        ...
        member.setCanEditMembers(true); member.setCanEditCoOwners(true); member.setCanKickMembers(true); member.setCanPromoteMembers(true); member.setCanDemoteMembers(true); member.setCanInvite(true); member.setCanSetWarps(true);
    }
    ...
    private static void notifyRankChange(ServerPlayerEntity actor, Team team, TeamPlayer target, boolean promoted) {
        var server = actor.getEntityWorld().getServer(); ServerPlayerEntity targetPlayer = server.getPlayerManager().getPlayer(target.getPlayerUuid()); String name = targetPlayer != null ? targetPlayer.getName().getString() : target.getPlayerUuid().toString();
    ...
    private static String resolveName(PlayerEntity viewer, TeamPlayer target) { if (viewer instanceof ServerPlayerEntity serverPlayer) { ServerPlayerEntity online = serverPlayer.getEntityWorld().getServer().getPlayerManager().getPlayer(target.getPlayerUuid()); if (online != null) return online.getName().getString(); } return target.getPlayerUuid().toString().substring(0, 8); }
