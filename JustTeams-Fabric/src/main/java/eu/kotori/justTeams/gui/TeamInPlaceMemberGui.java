...
public final class TeamInPlaceMemberGui {
    ...
    private static ItemStack bankPermissionItem(boolean withdrawEnabled,boolean autoBankPermission){ItemStack stack=new ItemStack(Items.GOLD_INGOT);stack.set(DataComponentTypes.CUSTOM_NAME,Text.literal("ʙᴀɴᴋ ᴡɪᴛʜᴅʀᴀᴡ ᴀɴᴅ ᴜsᴇ ᴀᴜᴛᴏʙᴀɴᴋ").setStyle(Style.EMPTY.withColor(Formatting.AQUA).withBold(true).withItalic(false)));boolean enabled=withdrawEnabled&&autoBankPermission;stack.set(DataComponentTypes.LORE,new LoreComponent(List.of(plainLine("Bank Withdraw and Autobank "+(enabled?"enabled":"disabled"),enabled?Formatting.GREEN:Formatting.RED))));return stack;}
