    ...
    private static final class Handler extends ScreenHandler {
        ...
        public void onSlotClick(int slot, int button, SlotActionType action, PlayerEntity player) {
            ...
            try {
                ...
                JustTeamsFabric.LOGGER.error("Failed to save blacklist removal", exception);
                return;
            }
        ...
        private int playerIndex(int slot) {
            for (int i = 0; i < PLAYER_SLOTS.length; i++) {
                if (PLAYER_SLOTS[i] == slot) return i;
            }
            return -1;
        }
